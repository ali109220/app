...
import { ActivityIndicator, AnimatedChart, AnimatedMetric, RevealOnScroll, STAGGER } from "@/site/motion";
...
const CHART_D = "M0 168 C64 146 88 122 130 134 C178 149 205 87 254 99 C305 112 344 72 391 84 C441 96 469 45 520 55";
const CHART_FILL_D = `${CHART_D} L520 210 L0 210Z`;

const SUPPORTING = ["Real-time visibility", "Connected workflows", "Role-based control", "Scalable operations"];
  ...
  return <div className="overflow-hidden rounded-2xl border" style={{ borderColor: T.border, background: T.panel }}>
    ...
    <div className="grid min-h-[430px] md:grid-cols-[180px_1fr]">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-3">{[['Portfolio','$2.45B'],['Activity','128.4K'],['Availability','99.9%']].map(([l,v],i)=><RevealOnScroll key={l} delay={i*STAGGER} className="rounded-xl border p-4" style={{ borderColor: T.border, background: T.bg }}><div className="text-xs tracking-wide" style={{ color: T.muted }}>{l}</div><div className="mt-3 text-xl font-semibold"><AnimatedMetric value={isAI && i===0 ? 'AI-ready' : isDigital && i===1 ? 'Omnichannel' : isManaged && i===2 ? 'Always-on' : v} /></div></RevealOnScroll>)}</div>
...
export default function ProductShowcase() {
  ...
  return <section className="relative border-b px-6 py-20 md:px-12 md:py-24" style={{ borderColor: T.border }} aria-labelledby="product-showcase-heading">
    <div className="mx-auto max-w-[1400px]">
      <RevealOnScroll className="grid gap-10 lg:grid-cols-12 lg:items-end"><div className="lg:col-span-7"><div className="font-jbmono text-xs uppercase tracking-[.22em]" style={{ color: T.signal }}>Product experience</div><h2 id="product-showcase-heading" className="mt-5 text-4xl font-bold leading-[.94] tracking-[-.03em] sm:text-5xl">See the platform, not just the promise</h2></div><p className="max-w-xl text-base leading-relaxed lg:col-span-5" style={{ color: T.muted }}>Explore representative product experiences that show how Tayseer’s capabilities come together in practice. These are design previews, not claims of a specific client deployment.</p></RevealOnScroll>
      <RevealOnScroll delay={STAGGER} className="mt-12 flex flex-wrap gap-2" role="tablist" aria-label="Product showcase">{PRODUCTS.map(p => <button key={p.id} type="button" role="tab" aria-selected={active===p.id} onClick={()=>setActive(p.id)} className="cta-secondary min-h-11 rounded-full border px-4 py-2 text-xs font-medium transition-colors" style={{ borderColor: active===p.id ? T.signal : T.border, color: active===p.id ? T.signal : T.muted, background: active===p.id ? T.panel : 'transparent' }}>{p.name}</button>)}</RevealOnScroll>
      <div className="mt-8 grid gap-10 lg:grid-cols-12 lg:items-center"><RevealOnScroll variant="fade-right" className="lg:col-span-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl" style={{ background: "rgba(13,90,140,.1)", color: T.signal }}><Icon size={21} aria-hidden="true" /></div><div className="mt-7 font-jbmono text-xs tracking-wide" style={{ color: T.signal }}>{product.eyebrow}</div><h3 className="mt-3 text-3xl font-semibold leading-tight tracking-[-.03em] sm:text-4xl">{product.title}</h3><p className="mt-5 text-sm leading-relaxed" style={{ color: T.muted }}>{product.copy}</p><Link href={product.to} className="group mt-7 inline-flex min-h-11 items-center gap-2 font-jbmono text-xs tracking-wide" style={{ color: T.signal }}>Explore {product.name} <ArrowUpRight size={13} aria-hidden="true" className="transition-transform group-hover:translate-x-1 group-hover:-translate-y-0.5" /></Link></RevealOnScroll><RevealOnScroll variant="scale" delay={STAGGER} className="lg:col-span-8"><MockWindow active={active} /></RevealOnScroll></div>
    </div>
  </section>;
}
