...
const PRODUCTS = [
  { id: "core", name: "Core Banking", eyebrow: "Modern foundation", title: "Run the bank on a core built for change.", copy: "A flexible banking foundation for transactions, integrations and continuous modernization.", to: "/solutions/core-banking", icon: Landmark },
    ...
    <div className="grid min-h-[430px] md:grid-cols-[180px_1fr]">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[1.4fr_.8fr]">
          ...
          <div className="space-y-3">{['Real-time visibility','Connected workflows','Role-based control','Scalable operations'].map((x,i)=><div key={x} className="rounded-xl border p-4" style={{ borderColor: T.border, background: T.bg }}><div className="flex items-center gap-3"><span className="flex h-7 w-7 items-center justify-center rounded-full text-[10px]" style={{ background: "rgba(13,90,140,.12)", color: T.signal }}>0{i+1}</span><span className="text-xs">{x}</span></div></div>)}</div>
        </div>
      </div>
    </div>
  </div>;
...
export default function ProductShowcase() {
  ...
  return <section className="relative border-b px-6 py-24 md:px-12" style={{ borderColor: T.border }} aria-labelledby="product-showcase-heading">
    <div className="mx-auto max-w-[1400px]">
      <div className="grid gap-10 lg:grid-cols-12 lg:items-end"><div className="lg:col-span-7"><div className="font-jbmono text-[11px] uppercase tracking-[.26em]" style={{ color: T.signal }}>Product experience</div><h2 id="product-showcase-heading" className="mt-5 text-4xl font-bold uppercase leading-[.94] tracking-[-.03em] sm:text-5xl">See the platform, not just the promise</h2></div><p className="max-w-xl text-base leading-relaxed lg:col-span-5" style={{ color: T.muted }}>Explore representative product experiences that show how Tayseer’s capabilities come together in practice. These are design previews, not claims of a specific client deployment.</p></div>
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-4"><div className="flex h-12 w-12 items-center justify-center rounded-xl" style={{ background: "rgba(13,90,140,.1)", color: T.signal }}><Icon size={21} aria-hidden="true" /></div><div className="mt-7 font-jbmono text-[10px] uppercase tracking-[.2em]" style={{ color: T.green }}>{product.eyebrow}</div><h3 className="mt-3 text-3xl font-semibold leading-tight tracking-[-.03em] sm:text-4xl">{product.title}</h3><p className="mt-5 text-sm leading-relaxed" style={{ color: T.muted }}>{product.copy}</p><Link href={product.to} className="mt-7 inline-flex min-h-11 items-center gap-2 font-jbmono text-[10px] uppercase tracking-widest" style={{ color: T.signal }}>Explore {product.name} <ArrowUpRight size={13} aria-hidden="true" /></Link></div><div className="lg:col-span-8"><MockWindow active={active} /></div></div>
    </div>
  </section>;
}
