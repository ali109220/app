...
import { RevealOnScroll, STAGGER, TIER, StageArrival, StagePulse, activityAlpha, useInView } from "@/site/motion";
      ...
      <div ref={flowRef} data-flow={flowInView ? "active" : undefined} className="mt-14 overflow-hidden rounded-2xl border" style={{ borderColor: T.border, background: T.panel }}>
        <div className="grid md:grid-cols-6">
          {FLOW.map((item,index) => {
            ...
            return <RevealOnScroll key={item.label} delay={TIER.visual + index*STAGGER} variant="scale" className="arch-node relative border-b p-5 text-center md:border-b-0 md:border-r" style={{ borderColor: T.border }}>
              ...
              {/* Green wash confirming the packet landed here, then gone. */}
              {index > 0 && <StageArrival className="inset-0 rounded-none" delay={arrival} cycle={FLOW_CYCLE} passes={FLOW_PASSES} style={{ background: `radial-gradient(circle at 50% 30%, ${activityAlpha(0.16)}, transparent 62%)` }} />}
              {!isLast && <>
                <StagePulse delay={departure} cycle={FLOW_CYCLE} passes={FLOW_PASSES} />
