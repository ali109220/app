...
import { RevealOnScroll, TIER, StageArrival, StagePulse, activityAlpha, useInView } from "@/site/motion";
    ...
    <div className="relative mx-auto max-w-[1400px]">
      ...
      <div className="mt-14 grid gap-8 lg:grid-cols-[.8fr_2.2fr]">
        {/* Scenario tabs are controls, not content: they appear immediately so
            they are clickable the instant the visitor sees them. */}
        ...
        <RevealOnScroll variant="scale" delay={TIER.visual} className="overflow-hidden rounded-2xl border" style={{ borderColor: T.border, background: T.bg }}>
          <div ref={flowRef} data-flow={flowInView ? "active" : undefined} className="grid gap-0 md:grid-cols-5">
            {STAGES.map((stage,index) => {
              ...
              return <div key={stage.label} className="arch-node relative border-b p-5 text-center md:border-b-0 md:border-r" style={{ borderColor: T.border }}>
                ...
                {index > 0 && <StageArrival className="inset-0" delay={(index - 1) * HOP_OFFSET + HOP_TRAVEL} cycle={FLOW_CYCLE} passes={FLOW_PASSES} style={{ background: `radial-gradient(circle at 50% 32%, ${activityAlpha(0.16)}, transparent 62%)` }} />}
                {!isLast && <>
                  <StagePulse delay={index * HOP_OFFSET} cycle={FLOW_CYCLE} passes={FLOW_PASSES} />
