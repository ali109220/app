  ...
  return (
    <section className="relative border-b px-6 py-20 md:px-12 md:py-24" style={{ borderColor: T.border }} aria-labelledby="transformation-story-heading">
      <div className="mx-auto max-w-[1400px]">
        <div>
          <RevealOnScroll delay={TIER.heading} className="font-jbmono text-xs uppercase tracking-[.22em]" style={{ color: T.signal }}>The transformation journey</RevealOnScroll>
          <div className="mt-5 grid gap-8 lg:grid-cols-12 lg:items-end">
            <RevealOnScroll as="h2" id="transformation-story-heading" delay={TIER.heading} className="text-4xl font-bold leading-[.94] tracking-[-.03em] sm:text-5xl lg:col-span-7">From legacy pressure to connected banking</RevealOnScroll>
            <RevealOnScroll as="p" delay={TIER.body} className="max-w-xl text-base leading-relaxed lg:col-span-5" style={{ color: T.muted }}>Modern banking change is not one project. It is a sequence of coordinated decisions across core systems, digital experiences, intelligence and operations.</RevealOnScroll>
          </div>
        </div>
        ...
        <div className="relative mt-14 grid gap-6 lg:mt-16 lg:grid-cols-3">
          ...
          {STEPS.map((step, index) => (
            <RevealOnScroll key={step.no} as="article" delay={TIER.support + index * STAGGER} className="motion-card motion-card-accent group relative h-full overflow-hidden rounded-2xl border p-7 sm:p-8" style={{ borderColor: T.border, background: index === 1 ? T.panel : T.bg }}>
              <div className="relative z-10 flex items-center justify-between gap-6">
                <div className="motion-card-icon flex h-12 w-12 items-center justify-center rounded-full border" style={{ borderColor: T.border, background: T.bg, color: T.signal }}><step.icon size={20} aria-hidden="true" /></div>
              ...
              <Link href={step.link} className="mt-8 inline-flex min-h-11 items-center gap-2 font-jbmono text-xs tracking-wide" style={{ color: T.signal }}>Explore <ArrowRight size={13} aria-hidden="true" className="motion-card-arrow" /></Link>
            </RevealOnScroll>
          ))}
        </div>

        <div className="mt-8 grid overflow-hidden rounded-2xl border md:grid-cols-3" style={{ borderColor: T.border }}>
          {/* Closing summary strip — a recap of the three cards above it. It
              carries no new idea, so it gets no reveal of its own. */}
