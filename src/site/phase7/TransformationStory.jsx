      ...
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="relative mt-14 grid gap-6 lg:mt-16 lg:grid-cols-3">
          ...
          {STEPS.map((step, index) => (
            <RevealOnScroll key={step.no} as="article" delay={index * STAGGER} className="motion-card motion-card-accent group relative h-full overflow-hidden rounded-2xl border p-7 sm:p-8" style={{ borderColor: T.border, background: index === 1 ? T.panel : T.bg }}>
        ...
        <div className="mt-8 grid overflow-hidden rounded-2xl border md:grid-cols-3" style={{ borderColor: T.border }}>
          {[['Legacy pressure','Disconnected systems · rising expectations'],['Connected change','Core platform · channels · intelligence · operations'],['Business outcome','More adaptable banking experiences']].map(([label,value],i) => <RevealOnScroll key={label} delay={i * STAGGER} className="p-6 md:p-7" style={{ background: i === 1 ? T.panel : T.bg, borderLeft: i ? `1px solid ${T.border}` : undefined }}><div className="font-jbmono text-xs tracking-wide" style={{ color: i === 1 ? T.signal : T.muted }}>{label}</div><div className="mt-3 text-sm font-medium leading-relaxed">{value}</div></RevealOnScroll>)}
        </div>
      </div>
    </section>
  );
